<template>
  <div>
    <h1>Pregunta: <button @click="getQuestion">Nueva pregunta</button></h1>
    <h2>{{ question }}</h2>
    <hr>
    <!-- <button>{{ correct }}</button> -->
    <button v-for="answer in answers" :disabled="isResponse" :class="{wrong : isResponse && answer == userResponse, correct : isResponse && answer == correct}" @click="checkResponse(answer)">{{ answer }}</button>
    <hr>
...
const emit = defineEmits(["QuestionResult"]);
  ...
  async function getQuestion() {
    try {
      ...
      console.log("Hay un error al buscar la pregunta: " + error);
